...
class FDUNet(nn.Module):
    def __init__(self,data_rows,data_cols,out_features):
        super().__init__()
        ...
        self.conv2 = nn.Conv2d(64,out_features,kernel_size=1,stride=1,padding='same')
        
    def forward(self, x):
        
        x1 = self.conv1(x) # (-1,32,nx,nx)
        x1 = self.db1(x1) # (-1.64,nx,nx)
        x2 = self.maxpool(x1) # (-1,64,nx/2,nx/2)
        x2 = self.db2(x2) # (-1,128,nx/2,nx/2)
        x3 = self.maxpool(x2) # (-1,128,nx/4,nx/4)
        x3 = self.db3(x3) # (-1,256,nx/4,nx/4)
        x4 = self.maxpool(x3) # (-1,256,nx/8,nx/8)
        x4 = self.db4(x4) # (-1,512,nx/8,nx/8)
        x5 = self.maxpool(x4) # (-1,512,nx/16,nx/16)
        x5 = self.db5(x5) # (-1,1024,nx/16,nx/16)
        
        x6 = self.up1(x5,x4) # (-1,256,nx/8,nx/8)
        x6 = self.db6(x6) # (-1,512,nx/8,nx/8)
        x7 = self.up2(x6,x3) # (-1,128,nx/4,nx/4)
        x7 = self.db7(x7) # (-1,256,nx/4,nx/4)
        x8 = self.up3(x7,x2) # (-1,64,nx/2,nx/2)
        x8 = self.db8(x8) # (-1,128,nx/2,nx/2)
        x9 = self.up4(x8,x1) # (-1,32,nx,nx)
        x9 = self.db9(x9) # (-1,64,nx,nx)
        x10 = self.conv2(x9) # (-1,out_features,nx,nx)
